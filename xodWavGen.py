...
class xodWavGen:
    ...
    def xodWTOsc(self, numSamples, shape, freqCtrl, phaseCtrl, quant='None'):
        ...
        if not (isinstance(freqCtrl, int) or isinstance(freqCtrl, float) or isinstance(freqCtrl, list) or isinstance(
                freqCtrl, np.ndarray)):
            print(
                'ERROR (xodWTOsc): freqCtrl must be a single freq value, a list, or a numpy array of frequency values')
            return 1
        ...
        if not (isinstance(phaseCtrl, int) or isinstance(phaseCtrl, float) or isinstance(phaseCtrl, list) or isinstance(
                phaseCtrl, np.ndarray)):
