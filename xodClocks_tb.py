...
# *****************************************************************************
#
# __::((xodClocks_tb.py))::__
#
# Python testbench for xodClocks
# required lib:
...
if xPlots == 1:
    ...
    pltXlabel = 'tbclkXbeat: '+str(Xnote)+' beats per bar'
    pltYlabel = 'Magnitude'

    # define a linear space from 0 to 1/2 Fs for x-axis:
    xaxis = np.linspace(0, tLen, tLen)
    
    xodplt.xodPlot1D(fnum, tbclkXbeat[0:tLen], xaxis,
                     pltTitle, pltXlabel, pltYlabel, 9, 3)
    
    fnum = 2
    pltTitle = 'Input Signal tbclkXbeatGnr (first '+str(tLen)+' samples)'
    pltXlabel = 'tbclkXbeatGnr: '+str(Xnote)+' beats per bar'
    pltYlabel = 'Magnitude'
    ...
    xodplt.xodPlot1D(fnum, .97*tbclkXpulseGnr[0:tLen], xaxis,
                     pltTitle, pltXlabel, pltYlabel, 9, 3)
